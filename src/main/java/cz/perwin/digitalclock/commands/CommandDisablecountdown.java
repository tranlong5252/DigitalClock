...
public class CommandDisablecountdown implements ICommand {
    ...
    public String reactBadArgsSize(String usedCmd) {
        return "§4" + DigitalClock.getMessagePrefix() + "§c Correct usage: '/" + usedCmd + " disablecountdown <name>'";
    }
    ...
    public void specialConditionProcess(DigitalClock main, Player player, String[] args) {
        player.sendMessage("§4" + DigitalClock.getMessagePrefix() + "§c This clock hasn't enabled countdown mode!");
    }
    ...
    public void process(DigitalClock main, Player player, String[] args) {
        ...
        player.sendMessage("§2" + DigitalClock.getMessagePrefix() + " §aYou have successfully disabled countdown mode on clock '" + args[1] + "'. This clock is now stopped, run it by command 'runclock <name>'.");
    }
}
