...
public class CommandFill extends MaterialCommand implements ICommand {
    ...
    public String reactBadArgsSize(String usedCmd) {
        return "§4" + DigitalClock.getMessagePrefix() + "§c Correct usage: '/" + usedCmd + " fill <name> <material id:data>'";
    }
    ...
    public void process(DigitalClock main, Player player, String[] args) {
        ...
        if (super.isValidName(args[2])) {
            var optional = XMaterial.matchXMaterial(args[2]);
            if (optional.isEmpty()) return;
            Material m = optional.get().parseMaterial();
            if (m.isSolid()) {
                player.sendMessage("§2" + DigitalClock.getMessagePrefix() + "§a Your clock '" + args[1] + "' changed filling material from " + oldmat + " to " + clock.setFillingMaterial(m.name(), 0).name().toLowerCase().replace("_", " "));
            } else {
            ...
            if (args[2].contains(":")) {
                ...
                if (super.isValidName(matdata[1])) {
                    if (super.isValidName(matdata[0])) {
                        ...
                        Material m = optional.get().parseMaterial();
                        if (m.isSolid()) {
                            player.sendMessage("§2" + DigitalClock.getMessagePrefix() + "§a Your clock '" + args[1] + "' changed filling material from " + oldmat + " to " + clock.setFillingMaterial(m.name(), Integer.parseInt(matdata[1])).name().toLowerCase().replace("_", " "));
                        } else {
                            player.sendMessage("§4" + DigitalClock.getMessagePrefix() + "§c You can't use " + m.name().toLowerCase().replace("_", " ") + " as a filling material, because it is not a block (cuboid shape)!");
                        }
                    } else {
                        try {
                            var optional = XMaterial.matchXMaterial(matdata[0]);
