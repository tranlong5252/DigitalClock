...
public class Version {
    ...
    public static void update(final CommandSender sender, final String prefix, String currentVersion, final String pluginName) {
        if (Version.updatingThread != null) {
            sendMessage(sender, "§2" + prefix + "§a Plugin is currently updating");
        } else {
            if (Version.getActualVersion().getVersion().equals(currentVersion)) {
                sendMessage(sender, "§4" + prefix + "§c Your version is actual so it can't be updated");
            } else {
                sendMessage(sender, "§2" + prefix + "§a Updating process started, please wait until it finishes");
                ...
                Version.updatingThread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            this.sendMessage("§2" + prefix + "§a Plugin has been updated. Use '/reload' command or restart the server");
                        } catch (Exception e) {
                            //e.printStackTrace();
                            this.sendMessage("§4" + prefix + "§c An error occured when updating plugin (" + e + ")");
                        } finally {
